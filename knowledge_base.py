...
def get_inventory_knowledge_base():
    ...
    rules.append(Rule("R59", "Action", lambda f: f.get('stock_status') in ['Low', 'Critically Low'] and f.get('anticipated_demand') == 'High', {'action': 'Standard Routine Reorder', 'priority_level': 'High'}, 0.85, "Standard operational signal triggered to return inventory to optimal thresholds under good demand."))
    rules.append(Rule("R60", "Action", lambda f: f.get('stock_status') in ['Low', 'Critically Low'] and f.get('supply_line_status') == 'Delayed', {'action': 'Buffer-Inflated Advance Order', 'priority_level': 'High'}, 0.80, "Stock is low and supplier pipelines are lagging; order early and increase order volume to offset future lag."))
    rules.append(Rule("R61", "Action", lambda f: f.get('stock_status') == 'Surplus' and f.get('financial_drain') == 'Severe' and f.get('operational_threat') != 'Spoilage Waste', {'action': 'Halt Ordering & Dynamic Markdown', 'priority_level': 'Low'}, 0.85, "Stop all procurement immediately; deploy aggressive promotional pricing to clear active financial overhead."))
    ...
    rules.append(Rule("R63", "Action", lambda f: f.get('warehouse_vacancy') == 'Full' and f.get('stock_status') != 'Critically Low' and f.get('operational_threat') != 'Spoilage Waste', {'action': 'Freeze Active Procurement', 'priority_level': 'Low'}, 0.90, "Physical space limits reached; completely freeze non-essential inventory incoming streams."))
    ...
    rules.append(Rule("R65", "Action", lambda f: f.get('stock_status') in ['Low', 'Critically Low'] and f.get('anticipated_demand') in ['High', 'Explosive'] and f.get('warehouse_vacancy') == 'Abundant', {'action': 'Economic Order Quantity (EOQ) Bulk Order', 'priority_level': 'Medium'}, 0.85, "Abundant space permits moving from routine orders to high-volume bulk discounts using warehouse optimization models."))
    rules.append(Rule("R66", "Action", lambda f: f.get('stock_status') in ['Low', 'Critically Low'] and f.get('warehouse_vacancy') == 'Full', {'action': 'Just-In-Time (JIT) Micro-Ordering', 'priority_level': 'Medium'}, 0.80, "Stock needs replenishment but the facility is gridlocked; order small micro-batches matching immediate delivery windows."))
    ...
    rules.append(Rule("R71", "Action", lambda f: f.get('stock_status') in ['Low', 'Critically Low'] and f.get('anticipated_demand') == 'Stagnant', {'action': 'Strategic Tactical Deferral', 'priority_level': 'Low'}, 0.70, "Stock drops below average benchmarks, but completely stagnant market demand alerts management to freeze ordering strings."))
    ...
    rules.append(Rule("R73", "Action", lambda f: f.get('supply_line_status') == 'Paralyzed' and f.get('stock_status') in ['Low', 'Critically Low'], {'action': 'Force-Route Alternative Sourcing', 'priority_level': 'CRITICAL'}, 0.95, "The primary supply channel is completely broken and stocks are low; force an immediate shift to unverified local backup providers."))
    rules.append(Rule("R74", "Action", lambda f: f.get('supply_line_status') == 'Slow But Stable' and f.get('stock_status') in ['Low', 'Critically Low'], {'action': 'Early Lifecycle Reorder Placements', 'priority_level': 'High'}, 0.85, "The transit pipeline is exceptionally slow but trustworthy; trigger a larger order early to accommodate extended timelines."))
